...
static int sd_set_sample_push_timing(struct rtsx_chip *chip)
{
	...
	if (CHK_SD_SDR104(sd_card) || CHK_SD_SDR50(sd_card)) {
		...
		if (chip->sd_sample_nodelay) {
			val = SD20_RX_POS_EDGE;
			RTSX_DEBUGP(("SD_SAMPLE_POINT_CTL value: 0x%x\n", val));
		}
...
static inline int sd_set_clock_divider(struct rtsx_chip *chip, u8 clk_div)
{
...
static int sd_change_bank_voltage(struct rtsx_chip *chip, u8 voltage)
{
	...
	mask = (REG_TUNED18 << shift) | SDBUS_IS_3V3;
	if (voltage == SD_IO_3V3) {
		if (chip->asic_code)
			val = (ASIC_3V3 << shift) | SD30_3V3;
		else
...
static int sd_perf_adjust(struct rtsx_chip *chip)
{
	struct sd_info *sd_card = &(chip->sd_card);
	int retval;
	int perf_adjust = 0;
	...
	if (rtsx_chk_nic_attach(chip) > 0) {
		if (rtsx_chk_perf_adjust_cr(chip))
			perf_adjust = 1;
	}

	RTSX_DEBUGP(("perf_adjust = %d\n", perf_adjust));

	if (perf_adjust)
		sd_card->sd_clock = chip->sd_sdr104_perf_adjust_clk;
	else
		sd_choose_proper_clock(chip);
...
int sd_switch_clock(struct rtsx_chip *chip)
{
	struct sd_info *sd_card = &(chip->sd_card);
	int retval;
	int re_tuning = 0, same_card = 0;
	
	if (chip->cur_card == SD_CARD)
		same_card = 1;

	if (chip->handshake_en) {
		if (chip->asic_code && same_card) {
			if (CHK_NIC_FIRST(chip))
				(void)sd_perf_adjust(chip);
		}
	}
	...
	if ((CHK_SD30_SPEED(sd_card) || CHK_MMC_DDR52(sd_card))) {
		if (same_card && (sd_card->sd_clock != chip->cur_clk))
			re_tuning = 1;
	}

	if (chip->handshake_en) {
		if (CHK_CR_FIRST(chip)) {
			if (CHK_SD_SDR104(sd_card) && !chip->perf_adjust_nic) {
				RTSX_DEBUGP(("Set perf_adjust_nic\n"));
				retval = rtsx_set_perf_adjust_nic(chip, 1);
				if (retval != STATUS_SUCCESS)
					TRACE_RET(chip, STATUS_FAIL);
				chip->perf_adjust_nic = 1;
			}
		}
	}
...
{
	RTSX_WRITE_REG(chip, CARD_PULL_CTL1, 0xFF, 0x65);
	RTSX_WRITE_REG(chip, CARD_PULL_CTL2, 0xFF, 0x55);
	RTSX_WRITE_REG(chip, CARD_PULL_CTL3, 0xFF, 0x95);
	RTSX_WRITE_REG(chip, CARD_PULL_CTL4, 0xFF, 0x09);
	RTSX_WRITE_REG(chip, CARD_PULL_CTL5, 0xFF, 0x05);
	RTSX_WRITE_REG(chip, CARD_PULL_CTL6, 0xFF, 0x04);
...
int sd_pull_ctl_enable(struct rtsx_chip *chip)
{
	...
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL3, 0xFF, 0xA9);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL4, 0xFF, 0x09);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL5, 0xFF, 0x09);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL6, 0xFF, 0x04);
...
static int sd_init_power(struct rtsx_chip *chip)
{
	...
	retval = rtsx_ldo_pwr_sel(chip, LDO_FROM_CARD);
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, STATUS_FAIL);
	}
#endif
...
#ifdef LDO_USING_CARD3V3
	RTSX_WRITE_REG(chip, CD_PAD_CTL, FORCE_CD_LOW_MASK, 0);
#endif
...
Switch_Fail:
	...
	if (chip->sdio_func_exist && try_sdio) {
		int rty_cnt = 0;
...
RTY_SD_RST:
	...
	if (hi_cap_flow) {
		RTSX_DEBUGP(("Check CCS bit, rsp[1] = 0x%x\n", rsp[1]));
...
SD_UNLOCK_ENTRY:
	...
	if (support_1v8) {
		if (CHK_SD_SDR104(sd_card))
			RTSX_WRITE_REG(chip, SD30_DRIVE_SEL, 0x07, chip->sdr104_drive_sel_1v8);
		else
	...
	if (retval != STATUS_SUCCESS) {
		if (CHK_SD30_SPEED(sd_card) && !sd20_mode) {
			retval = sd_init_power(chip);
			if (retval != STATUS_SUCCESS) {
				TRACE_RET(chip, STATUS_FAIL);
			}
...
RW_FAIL:	
	...
	if (sd_check_err_code(chip,SD_CRC_ERR)) {
		RTSX_DEBUGP(("chip->rw_retry_cnt = %d\n", chip->rw_retry_cnt));
		if (CHK_MMC_4BIT(sd_card) || CHK_MMC_8BIT(sd_card)) {
			...
			if (CHK_SD30_SPEED(sd_card))
				chip->sd20_mode = 1;
			if (chip->rw_retry_cnt == 2)
				chip->sd_sample_nodelay = 1;
			retval = reset_sd_card(chip);
			if (retval != STATUS_SUCCESS) {
				chip->card_ready &= ~SD_CARD;
				chip->card_fail |= SD_CARD;
...
int sd_power_off_card3v3(struct rtsx_chip *chip)
{
	...
	if (chip->asic_code) {
		...
		RTSX_WRITE_REG(chip, FPGA_PULL_CTL, 
			FPGA_SD_CD_PULL_CTL_MASK | FPGA_MS_CD_PULL_CTL_MASK | FPGA_SD_PULL_CTL_MASK,
			FPGA_SD_CD_PULL_CTL_EN | FPGA_MS_CD_PULL_CTL_EN | FPGA_SD_PULL_CTL_DIS);
	}
...
#ifdef LDO_USING_CARD3V3
	retval = rtsx_ldo_pwr_sel(chip, LDO_FROM_DV33);
	if (retval != STATUS_SUCCESS) {
		TRACE_RET(chip, STATUS_FAIL);
	}
#endif
