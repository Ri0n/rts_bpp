...
{
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL1, 0xFF, 0x65);
	...
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL5, 0xFF, 0x05);
...
static void xd_fill_pull_ctl_enable(struct rtsx_chip *chip)
{
	...
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL2, 0xFF, 0x55);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL3, 0xFF, 0x95);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL4, 0xFF, 0x05);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL5, 0xFF, 0x05);
	rtsx_add_cmd(chip, WRITE_REG_CMD, CARD_PULL_CTL6, 0xFF, 0x04);
}
...
static int reset_xd(struct rtsx_chip *chip)
{
	...
	if (chip->asic_code) {
		...
		rtsx_add_cmd(chip, WRITE_REG_CMD, FPGA_PULL_CTL, 
				FPGA_SHARE_CD_PULL_CTL_MASK | FPGA_XD_PULL_CTL_MASK,
				FPGA_SHARE_CD_PULL_CTL_DIS | FPGA_XD_PULL_CTL1_EN | 
				FPGA_XD_PULL_CTL2_DIS | FPGA_XD_PULL_CTL3_EN);
	}
	...
	if (!chip->ft2_fast_mode) {
		...
		if (chip->asic_code) {
			...
			rtsx_add_cmd(chip, WRITE_REG_CMD, FPGA_PULL_CTL, 
				     FPGA_SHARE_CD_PULL_CTL_MASK | FPGA_XD_PULL_CTL_MASK,
				     FPGA_SHARE_CD_PULL_CTL_DIS | FPGA_XD_PULL_CTL1_EN | 
				     FPGA_XD_PULL_CTL2_EN | FPGA_XD_PULL_CTL3_DIS);
		}
	...
	rtsx_add_cmd(chip, WRITE_REG_CMD, CD_PAD_CTL, FORCE_CD_LOW_MASK, 0);
#endif
...
int xd_power_off_card3v3(struct rtsx_chip *chip)
{
	...
	if (chip->asic_code) {
		...
		RTSX_WRITE_REG(chip, FPGA_PULL_CTL, 
				     FPGA_SHARE_CD_PULL_CTL_MASK | FPGA_XD_PULL_CTL_MASK,
				     FPGA_SHARE_CD_PULL_CTL_EN | FPGA_XD_PULL_CTL_DIS);
	}
